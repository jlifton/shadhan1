...
import {
  MatDialog, MatDialogConfig, MatPaginator, MatSnackBar, MatSort, MatSortable, MatTableDataSource, PageEvent,
  SortDirection
...
export class ArchivesTableComponent implements OnInit {
  rows: SingleDTO[];
  archives: SingleDTO[] = new Array<SingleDTO>();
  ...
  getArchives(defaultSort) {
    this.singlesService.getArchives().subscribe(
      (data: SingleDTO[]) => {
        this.archives = data;
         let previousFilter = '';
         ...
         this.dataSource = new MatTableDataSource(< SingleDTO[] > this.archives);
  ...
  handleSortAccessor(item, property) {
    switch (property) {
      case 'identity.firstName':
        return item.identity.firstName.toLowerCase();
      case 'identity.lastName':
        return item.identity.lastName.toLowerCase();
      case 'identity.sex':
        return item.identity.sex.toLowerCase();
      case 'identity.age':
        return item.identity.age;
      case 'identity.maritalStatus':
        return item.identity.maritalStatus.toLowerCase();

      case 'religioEthnic.hashkafa':
        return item.religioEthnic.hashkafa.toLowerCase();
      case 'religioEthnic.ethnicityAdditional':
        return item.religioEthnic.ethnicityAdditional.toLowerCase();
      case 'religioEthnic.ethnicity':
        return item.religioEthnic.ethnicity.toLowerCase();
      case 'religioEthnic.convert':
        return item.religioEthnic.convert;
      case 'religioEthnic.cohen':
        return item.religioEthnic.cohen;
      case 'religioEthnic.primaryActivity':
        return item.religioEthnic.primaryActivity.toLowerCase();

      case 'residence.city':
        return item.residence.city.toLowerCase();
      case 'residence.country':
        return item.residence.country.toLowerCase();

      case 'physical.smoker':
        return item.physical.smoker;
      case 'physical.height':
        return item.physical.height;
      case 'physical.build':
        return item.physical.build.toLowerCase();
      case 'physical.description':
        return item.physical.description.toLowerCase();

      case 'contact.name':
        return item.contact.name.toLowerCase();
      case 'contact.relationship':
        return item.contact.relationship.toLowerCase();
      case 'contact.primaryPhone':
        return item.contact.primaryPhone.toLowerCase();
      case 'contact.secondaryPhone':
        return item.contact.secondaryPhone.toLowerCase();
      case 'contact.email':
        return item.contact.email.toLowerCase();

      case 'source.name':
        return item.source.name.toLowerCase();
      case 'source.email':
        return item.source.email.toLowerCase();
      case 'source.phone':
        return item.source.phone.toLowerCase();

      case 'occupation':
        return item.occupation.toLowerCase();
      case 'specialNeeds':
        return item.specialNeeds.toLowerCase();
      case 'personalityRequirements':
        return item.personalityRequirements.toLowerCase();
      case 'pastEducation':
        return item.pastEducation.toLowerCase();
      case 'dateEntered':
        return item.dateEntered;
      case 'comments':
        return item.comments.toLowerCase();
  ...
  deleteArchive(row: SingleDTO) {
    ...
    if (confirm('Delete Archived Single ' + row.identity.firstName + ' ' + row.identity.lastName + ' ?')) {
      this.singlesService.deleteArchive(row._id).subscribe(
        data => {
